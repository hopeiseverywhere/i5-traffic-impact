...
def normalize_direction(value: str) -> str:
    ...
    if v == "i": return "N"   # increasing = northbound
    if v == "d": return "S"   # decreasing = southbound
    if v == "a": return "N"   # ahead
    if v == "b": return "S"   # back
    return None

def detect_mile_latlon_columns(mileposts: pd.DataFrame):
    """Detect columns that look like milepost, lat, lon."""
    mile_col = next((c for c in mileposts.columns if "mile" in c.lower()), None)
